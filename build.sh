...
if [ "$1" == "rpm" ]; then
    ...
    if [ -e RedhawkDevUtils.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/RedhawkDevUtils_v1-3.1.0
        tar czf ${tmpdir}/RedhawkDevUtils_v1-3.1.0.tar.gz --exclude=".svn" -C ${tmpdir} RedhawkDevUtils_v1-3.1.0
        rpmbuild -ta ${tmpdir}/RedhawkDevUtils_v1-3.1.0.tar.gz
        rm -rf $tmpdir
    else
